...
class LoRALinear(torch.nn.Module):
    ...
    def forward(self, x):
        return (self.dropout(x) @ self.B.t()) @ self.A.t() * self.scaling
...
class GPT2SentimentClassifier(torch.nn.Module):
    def __init__(self, config):
        ...
        self.lora = LoRALinear(self.hidden_size, self.hidden_size, r=8, alpha=16, dropout=config.hidden_dropout_prob)
...
class SentimentDataset(Dataset):
    ...
    def pad_data(self, data):
        ...
        return enc['input_ids'], enc['attention_mask'], labels, sents, sent_ids

    def collate_fn(self, all_data):
        token_ids, attn_mask, labels, sents, sent_ids = self.pad_data(all_data)
        return {
            'token_ids': token_ids,
...
def load_data(filename, flag='train'):
    ...
    with open(filename, 'r') as fp:
        for record in csv.DictReader(fp, delimiter='\t'):
            sent, sent_id = record['sentence'].lower().strip(), record['id'].lower().strip()
            if flag == 'test':
...
def model_eval(dataloader, model, device):
    ...
    for batch in tqdm(dataloader, desc='eval', disable=TQDM_DISABLE):
        ...
        y_pred.extend(preds)
        if b_labels is not None: y_true.extend(b_labels.flatten())
...
def train(args):
    ...
    param_groups = [
        {"params": [p for n, p in model.named_parameters() if "gpt" in n], "lr": args.lr * 0.5},
        {"params": [p for n, p in model.named_parameters() if "classifier" in n], "lr": args.lr},
        {"params": [p for n, p in model.named_parameters() if "lora" in n], "lr": args.lr}
    ]
...
def test(args):
    ...
    _, test_f1 = model_eval(test_loader, model, device)
