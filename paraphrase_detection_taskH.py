...
class ParaphraseGPT(nn.Module):
    ...
    def forward(self, input_ids, attention_mask):
        ...
        # Mixed: 0.7 * mean + 0.3 * last
        mixed_hidden = 0.7 * mean_hidden + 0.3 * last_hidden

        # Multi-sample dropout
        logits = 0
        for _ in range(3):
            logits += self.paraphrase_detection_head(self.dropout(mixed_hidden))
        return logits / 3
...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm(para_train_dataloader, desc=f'train-{epoch}', disable=TQDM_DISABLE):
            ...
            loss = smooth_cross_entropy(logits, labels, smoothing=0.05)
            loss.backward()
            optimizer.step()
...
def test(args):
    ...
    para_dev_data = ParaphraseDetectionDataset(load_paraphrase_data(args.para_dev), args)
    para_test_data = ParaphraseDetectionTestDataset(load_paraphrase_data(args.para_test, split='test'), args)
...
if __name__ == "__main__":
    args = get_args()
    args.filepath = f'{args.epochs}-{args.lr}-paraphrase_taskH.pt'
